...
(define (merge comp list1 list2)
  ...
  (define (helper lst lst1 lst2)
    (cond ((and (null? lst1) (null? lst2)) lst)
          ((null? lst2) (append lst lst1))
          ((null? lst1) (append lst lst2))
          ((comp (car lst1) (car lst2)) 
            (helper (append lst (list (car lst1))) (cdr lst1) lst2))
          (else 
            (helper (append lst (list (car lst2))) lst1 (cdr lst2))) )
    )
  (helper '() list1 list2))
...
(define (patterns)
  ...
  (define (has-both lst) 
    (and (member? 'over lst) (member? 'under lst)))

  (define (add-over s) (append (list 'over) s))
  (define (add-under s) (append (list 'under) s))

  (define helper-stream
    (cons-stream '() 
      (interleave (stream-map add-over helper-stream)
        (stream-map add-under helper-stream) )))

  (stream-filter has-both helper-stream)
)
