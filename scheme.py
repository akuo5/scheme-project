...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if formals.length() == vals.length():
            ...
            raise SchemeError("different number of formal \
...
class Stream(SchemeValue):
    ...
    def stream_cdr(self):
        """ Gets the memorized value of the rest of the stream. If the rest has
...
def do_cons_stream_form(vals, env):
    ...
    new_stream = Stream(scheme_eval(first, env), rest, env)
...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            # PROBLEM 12
            new_frame = self.env.make_call_frame(self.formals, args)
            evaluated = scheme_eval(self.body, new_frame)
...
class NuProcedure(LambdaProcedure):
    ...
    Special syntax: (nu (*formals*) *body*)
    """
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            value, n = do_lambda_form(Pair(target.second, vals.second), env)
...
def do_let_form(vals, env):
    ...
    # PROBLEM A16
    lambform = Pair(Pair(lambda_sym, Pair(names, exprs)), values)
    return lambform, env
...
def do_if_form(vals, env):
    ...
    # PROBLEM A13
    cond, result = vals[0], vals.second
    conseq = result[0]
    
    evaluated = scheme_eval(cond, env)
    if evaluated:
        return conseq, env
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            if body.length() == 0:
                ...
                return Pair(begin_sym, body), env
            return body.first, env
...
def check_formals(formals):
    ...
    current_lst = []
    while formals.length() != 0:
        if not scheme_symbolp(formals.car()):
            raise SchemeError
        elif len(current_lst) > 1 and formals.car() in current_lst:
            raise SchemeError
        current_lst += [formals.car()]
        formals = formals.cdr()
    return
