...
class SchemeValue:
    ...
    def stream_car(self):
        bad_type(self, 0, "stream-car")

    def stream_cdr(self):
        bad_type(self, 0, "stream-cdr")
...
class Pair(SchemeValue):
    ...
    class ListIterator:
        """ Creates an Iterator through a well-formed list"""
    ...
    def __iter__(self):
        if not self._list_end().nullp():
            raise SchemeError("iterator attempted on improper list")
...
def scheme_stream_nullp(x):
    return x.stream_nullp()

@primitive("stream-car")
def scheme_stream_car(x):
    return x.stream_car()
