...
;expect Pair('quote', Pair('thisistest', nil))
...
'hellothisisasymbol
;expect hellothisisasymbol

(car (cdr '(je suis un petit mounton)))
;expect suis
...
(define Audi (/ (+ yolo 3) 2))
;expect Audi

Audi
;expect 13

(begin (display yolo) (newline) 'swag)
;expect 21
;expect swag

(begin (print Audi) 'MDR)
;expect 13
;expect mdr

(lambda (z y) (+ z z z y))
;expect (lambda (z y) (+ z z z y))

(lambda () ('ROFL))
;expect (lambda () ('rofl))

(define f (lambda () 'ROFL))
;expect f
...
(define (f rofl) (* 256 rofl))
;expect f

(f rofl)
;expect 512

(if (= (remainder 52 26) 0) #t #f)
;expect #t

(if (equal? 'yolo 'swag) (+0 3) (- 25 2))
;expect 23

(or (equal? 'yolo 'swag) (= 2 2))
;expect #t

(or (= 2 2) (equal? 'yolo 'swag))
;expect #t
...
(cond ((equal? 'yolo 'yolo) 'swag!)
      (else 'noswag))
;expect swag!

(cond ((equal? 'swag 'yoloswag) 'swagswagswag)
      ((= 4 2) 'wat)
      (else 'hahnoswag))
;expect hahnoswag
...
(define z (mu (x) (remainder x y)))
;expect z

(define b (lambda (x y) (z (+ x x))))
;expect b
